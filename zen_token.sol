...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    uint256 private constant HEARTS_PER_ZEN = 10 ** uint256(decimals); // 1e8
    uint256 private constant ZEN_PER_BTC = 1e4;
    uint256 private constant SATOSHIS_PER_BTC = 1e8;
    uint256 internal constant HEARTS_PER_SATOSHI = HEARTS_PER_ZEN / SATOSHIS_PER_BTC * ZEN_PER_BTC;
    ...
    /* Seed for WAAS Lobby */
    uint256 internal constant WAAS_LOBBY_SEED_ZEN = 1e9;
    uint256 internal constant WAAS_LOBBY_SEED_HEARTS = WAAS_LOBBY_SEED_ZEN * HEARTS_PER_ZEN;
    ...
    uint256 private constant BPB_MAX_ZEN = 150 * 1e6;
    uint256 internal constant BPB_MAX_HEARTS = BPB_MAX_ZEN * HEARTS_PER_ZEN;
    ...
    uint8 internal constant ETH_ADDRESS_ZEN_LEN = ETH_ADDRESS_BYTE_LEN * 2;

    uint8 internal constant CLAIM_PARAM_HASH_BYTE_LEN = 12;
    uint8 internal constant CLAIM_PARAM_HASH_ZEN_LEN = CLAIM_PARAM_HASH_BYTE_LEN * 2;

    uint8 internal constant BITCOIN_SIG_PREFIX_LEN = 24;
    bytes24 internal constant BITCOIN_SIG_PREFIX_STR = "Bitcoin Signed Message:\n";

    bytes internal constant STD_CLAIM_PREFIX_STR = "Claim_ZEN_to_0x";
    bytes internal constant OLD_CLAIM_PREFIX_STR = "Claim_BitcoinZEN_to_0x";

    bytes16 internal constant ZEN_DIGITS = "0123456789abcdef";
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "ZEN: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "ZEN: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "ZEN: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "ZEN: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "ZEN: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "ZEN: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "ZEN: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "ZEN: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "ZEN: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "ZEN: Empty stake list");
        require(stakeIndex < stakeListRef.length, "ZEN: stakeIndex invalid");
            ...
            if (prevUnlocked) {
                ...
                if (servedDays > st._stakedDays) {
                    ...
                    if (servedDays < MIN_AUTO_STAKE_DAYS) {
                        require(!st._isAutoStake, "ZEN: Auto-stake still locked");
                    }
                }
            }
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "ZEN: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "ZEN: newStakedHearts must be at least minimum shareRate");
        ...
        returns (uint256 bonusHearts)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 150M ZEN of stake.
...
contract UTXOClaimValidation is StakeableToken {
        ...
        returns (bool)
    {
        ...
        require(day >= CLAIM_PHASE_START_DAY, "ZEN: Claim phase has not yet started");
        require(day < CLAIM_PHASE_END_DAY, "ZEN: Claim phase has ended");
    ...
    function claimMessageMatchesSignature(
        ...
        returns (bool)
    {
        require(v >= 27 && v <= 30, "ZEN: v invalid");
        ...
        returns (bytes20)
    {
        ...
        */
        ...
        if (compressed) {
            ...
            require(!(nested && bech32), "ZEN: claimFlags invalid");
        ...
        if (nested) {
            return _hash160(abi.encodePacked(zen"0014", pubKeyHash));
        }
        return pubKeyHash;
    }
        ...
        returns (bool)
    {
        /*
            ...
            internal node will never have the zero-fill, as guaranteed by ZEN's Merkle
            tree construction.
            ...
            ZEN's Merkle tree construction, which eliminates the only edge-case where
            this validation would not apply.
        */
        require((uint256(proof[0]) & MERKLE_LEAF_FILL_MASK) == 0, "ZEN: proof invalid");
        for (uint256 i = 1; i < proof.length; i++) {
            require((uint256(proof[i]) & MERKLE_LEAF_FILL_MASK) != 0, "ZEN: proof invalid");
        }
        ...
        returns (bytes memory)
    {
        ...
        if (claimParamHash == 0) {
            return abi.encodePacked(
                ...
                uint8(prefixStr.length) + ETH_ADDRESS_ZEN_LEN,
                prefixStr,
                addrStr
            );
        }

        bytes memory claimParamHashStr = new bytes(CLAIM_PARAM_HASH_ZEN_LEN);

        _zenStringFromData(claimParamHashStr, claimParamHash, CLAIM_PARAM_HASH_BYTE_LEN);

        return abi.encodePacked(
            BITCOIN_SIG_PREFIX_LEN,
            BITCOIN_SIG_PREFIX_STR,
            uint8(prefixStr.length) + ETH_ADDRESS_ZEN_LEN + 1 + CLAIM_PARAM_HASH_ZEN_LEN,
            prefixStr,
            addrStr,
            "_",
            claimParamHashStr
    ...
    {
        addrStr = new bytes(ETH_ADDRESS_ZEN_LEN);
        _zenStringFromData(addrStr, bytes32(bytes20(addr)), ETH_ADDRESS_BYTE_LEN);
    ...
    function _zenStringFromData(bytes memory zenStr, bytes32 data, uint256 dataLen)
        private
        pure
    {
        uint256 offset = 0;
        ...
        for (uint256 i = 0; i < dataLen; i++) {
            ...
            zenStr[offset++] = ZEN_DIGITS[b >> 4];
            zenStr[offset++] = ZEN_DIGITS[b & 0x0f];
        }
    }
...
contract UTXORedeemableToken is UTXOClaimValidation {
    ...
    {
        /* Sanity check */
        require(rawSatoshis <= MAX_BTC_ADDR_BALANCE_SATOSHIS, "ZEN: CHK: rawSatoshis");

        /* Enforce the minimum stake time for the auto-stake from this claim */
        require(autoStakeDays >= MIN_AUTO_STAKE_DAYS, "ZEN: autoStakeDays lower than minimum");
        ...
        require(!btcAddressClaims[btcAddr], "ZEN: BTC address balance already claimed");

        /* Ensure BTC address is part of the Merkle tree */
        require(
            _btcAddressIsValid(btcAddr, rawSatoshis, proof),
            "ZEN: BTC address or balance unknown"
        );
        ...
        returns (uint256 totalClaimedHearts)
    {
        ...
        require(g._currentDay >= CLAIM_PHASE_START_DAY, "ZEN: Claim phase has not yet started");
        require(g._currentDay < CLAIM_PHASE_END_DAY, "ZEN: Claim phase has ended");
...
contract TransformableToken is UTXORedeemableToken {
        ...
        payable
    {
        ...
        require(enterDay < CLAIM_PHASE_END_DAY, "ZEN: Lobbies have ended");

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "ZEN: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "ZEN: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "ZEN: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "ZEN: count invalid");
        }
    ...
    function xfLobbyFlush()
        external
    {
        require(address(this).balance != 0, "ZEN: No value");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "ZEN: Param invalid");
...
contract ZEN is TransformableToken {
    constructor()
        public
    {
